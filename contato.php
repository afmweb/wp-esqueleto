...
$emailEnviado = FALSE;
$contatoEmail = filter_input(INPUT_POST,  'contato-email' , FILTER_SANITIZE_STRING);
//$contatoMensagem = filter_input(INPUT_POST, 'contato-mensagem', FILTER_SANITIZE_STRING);
if( isset($_POST['form-contato-submeter']) ){
    ...
    if( trim($contatoMensagem) === '' || strlen($contatoMensagem)< 3 ){
        $erroContato = 'Entre com a mensagem.';
    }
    ...
    }elseif( ! filter_var( trim($_POST['contato-email']), FILTER_VALIDATE_EMAIL)){
    ...
    if( is_null($erroContato) ){
        ...
        $body = "Name: $contatoNome \n\nEmail: $contatoEmail \n\nComments: $contatoMensagem";

          wp_mail('marcelinoandre@gmail.com', $assunto, $body);/// mail(..., ..., ...); -> Aqui a função para envio do e-mail. Como estamos localmente, a mail() não funciona.
